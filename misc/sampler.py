...
class CartesianSeqSampler(CartesianSampler):
    def __init__(self, M, L, n, batch_size, device="cpu", train=True, overlap=True):
        ...
        Attributes:
            ...
            train: if True randomly sample a subset else ordered sample. (default: True)
            overlap: allow overlapping between sampled sequences in the test phase. (default: True)
    ...
    def __next__(self):
        if self.training:
            ...
            if self.overlap:
                ...
                next_idx = prev_idx + self.batch_size
                isLast = next_idx > self.test_size
                if isLast:
                    batch_size = self.test_size - prev_idx
